...
public class MushroomController {
    ...
    public MushroomController(MushroomLexiconAddService mushroomLexiconAddService, MushroomLexiconService mushroomLexiconService) {
    ...
    public String addMushroom(@RequestParam("picture") MultipartFile picture, @ModelAttribute Mushroom mushroom) throws IOException {
        if (!picture.isEmpty()) {
            try {
                picture.transferTo(new File("src/main/resources/static/pictures/" + picture.getOriginalFilename()));
                mushroom.setPicture(picture.getOriginalFilename());
